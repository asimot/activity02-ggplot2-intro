...
- `species`: factor denoting penguin species (Adelie, Chinstrap, and Gentoo)
- `island`: factor denoting island in Palmer Archipelago, Antarctica (Biscoe,             Dream, or Torgersen)
- `bill_length_mm`: Number denoting bill length (millimeters)
- `bill_depth_mm`: Number denoting bill depth (millimeters)
- `flipper_length_mm`: Integer denoting flipper length (millimeters)
- `body_mass_g`: Integer denoting body mass (grams)
- `sex`: Factor denoting penguin sex (female, male)
- `year`: Integer denoting the study year (2007, 2008, or 2009)
...
**Response**: The three boxplots that represent each unique species now has a 
  unique color to identify the individual species easier.
...
**Response**: The boxplot shows the distribution better and the correlated 5 
  number summary to easily see the median and quartile ranges of flipper length.
  The jitter plot easily shows the individual points so that we can see each 
  point rather than a summary of the points.
...
**Response**: The R markdown file looks significantly better and easier to read
  compared to the markdown file.
...
**Response**: Since layering is dependent upon order, if we use geom_boxplot 
  before geom_jitter, the boxplot will sit underneath the jitter points, thus 
  we can see all the points fro geom_jitter.  If we invert this selection the
  boxplots cover up the points that exist behind the boxplot.

Therefore, the order of geom_boxplot then geom_jitter is preferred to see all the points in the dataset.
...
**Response**: We change colors based on which layer we are interacting with by 
  adding a map to the geom_ layers.  Adding the color mapping in the ggplot layer
  effects the colors in all layers; whereas in the mappings for each geom_ layer 
  we can change individual colors of a layer.  Doing them individually is 
  beneficial to have more customization, but the general use of it in ggplot 
  is helpful as a global changes.
